...
// Send reboot signal when clicked.
// Don't do this in production!
const srvButton=(yomo,srv)=>{
  try {
    yomoClock(yomo,yomo.state().boot[srv]||0)
    || bridge(yomo,{peerId:srv,fname:'boot'});
  } catch(e) {}
  return {
...
const boot=(state={},action)=>{
  switch(action.type) {
    case 'boot': return {...state,[action.srv]:action.time};
  }
  return state;
}

const msgClient=combineReducers({me,peer,msg,pipes,boot});
